...
export default function LoanProductsPage() {
	...
	const canViewLoans = permissions.canView("LOANS");
	const canCreateLoan = permissions.canCreate("LOANS");
	const canUpdateLoan = permissions.canUpdate("LOANS");
	const canDeleteLoan = permissions.canDelete("LOANS");
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (editingId && !canUpdateLoan) {
			toast({
				title: "Access Denied",
				description: "You don't have permission to update loan products",
				variant: "destructive",
			});
			return;
		}

		if (!editingId && !canCreateLoan) {
			toast({
				title: "Access Denied",
				description: "You don't have permission to create loan products",
				variant: "destructive",
			});
			return;
		}
	...
	const handleEdit = (product: LoanProduct) => {
		if (!canUpdateLoan) {
			toast({
				title: "Access Denied",
				description: "You don't have permission to edit loan products",
				variant: "destructive",
			});
			return;
		}
		...
		setFormData({
			...
			minTotalContributions: product.minTotalContributions,
		});
	...
	const handleDelete = async (id: number) => {
		if (!canDeleteLoan) {
			toast({
				title: "Access Denied",
				description: "You don't have permission to delete loan products",
				variant: "destructive",
			});
			return;
		}
				...
				<Card className="w-full max-w-md">
					...
					<CardContent>
						<p className="text-muted-foreground">
							You don't have permission to view loan products. Please contact
							your administrator.
						</p>
					</CardContent>
				</Card>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								...
								<div className="space-y-2">
									...
									<Input
										id="maxSalaryMonths"
										type="number"
										value={formData.minTotalContributions}
										onChange={(e) =>
											setFormData({
												...formData,
												minTotalContributions: Number.parseInt(e.target.value),
											})
										}
										required
									/>
							...
							<CardContent>
								<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
									...
									<div>
										...
										<p className="text-lg font-semibold">
											ETB {product.minTotalContributions}
										</p>
									</div>
								</div>
							</CardContent>
